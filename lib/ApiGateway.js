...
export class ApiGateway {
  ...
  async #parse(response) {
    ...
    try {
      if (!this.#options.parser) return await response.json();
      return await this.#options.parser(response);
    } catch (e) {
      return null;
    }
  }
  ...
  async request(options) {
    const { headers, path, signal, ...rest } = options;
    ...
    if (!res.ok) {
      NetworkError.throw(
        `ApiGateway ${this.#options.name ?? ''} request failed`,
        {
          ...
          url,
          details: await this.#parse(res),
        },
      )
    }
    if (options.parseBody) return await this.#parse(res);
    return null;
  }
